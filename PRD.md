...
| Термин | Определение |
| --- | --- |
| Workspace | Изолированный tenant компании или команды. Граница данных, участников и оплаты; содержит проекты. |
...
1. Пользователь создаёт Workspace, Project и выбирает шаблон (greenfield, новая feature, modernization, integration, compliance).
...
- Workspace как жёсткая граница данных; Account может состоять в нескольких Workspace.
- Project внутри Workspace с понятным переключением контекста.
...
- MCP Gateway применяет Workspace & Access Control, data classification и policy до выполнения любого resource/tool request; доступ к одному Project не даёт доступ к соседним Project того же tenant.
...
| --- | --- | --- |
| **Identity & Account Management** | Account, Credential, login method, session, MFA factor, password recovery, consent, PlatformRole | Workspace, membership, роли и права внутри tenant |
| **Workspace & Access Control** | Workspace, Project, Project Template, Membership, Team, Role, Permission, RoleAssignment, Invitation, access policy, tenant boundary, project lifecycle | Учётные данные, платформенные роли, содержимое требований, доменную модель, delivery backlog |
...
- MCP Gateway выдаёт внешнему агенту только policy-approved versioned projections через Integration Hub; он не обходит Workspace & Access Control и не является внутренним Agent Runtime.
...
1. **Identity & Account Management**, **Workspace & Access Control**, **Governance & Compliance** — tenancy, authorization, audit и project boundary до первого бизнес-артефакта.
...
- Активные проекты, weekly active collaborators, retention по Workspace.
